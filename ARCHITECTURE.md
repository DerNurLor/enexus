...
NCFU Schedule Bot — **микросервисная** система из 4 прикладных сервисов на общей инфраструктуре (MongoDB + Redis), за единой точкой входа (Nginx). Каждый сервис — отдельный Docker-контейнер с собственным FastAPI-приложением.
...
└───────────────────────────────┬──────────────────────────────────────────┘
           ...
           │                  │                │                  │
    ┌──────▼──────┐   ┌───────▼──────┐ ┌──────▼──────┐  ┌───────▼──────┐
    │   backend   │   │     bot      │ │   miniapp   │  │  dashboard   │
    │    :8000    │◄──│    :8001     │ │    :8002    │  │    :8003     │
    │             │   │              │ │             │  │              │
    │ REST API    │   │ aiogram 3    │ │ FastAPI API │  │ FastAPI API  │
    │ GraphQL     │   │ OpenAI NLU   │ │ React SPA   │  │ React SPA    │
    │ Scraper     │   │ Webhook      │ │ Mini App    │  │ Admin Panel  │
...
4. **dashboard** работает с обеими БД и должен быть изолирован от пользовательского трафика
...
╠═══════════════╬════════════════════════╬═══════════════════════════════╣
║ dashboard     ║ —                      ║ Обе БД напрямую (read/write)  ║
╚═══════════════╩════════════════════════╩═══════════════════════════════╝
```
...
/dashboard/*          →  dashboard:8003 (rate: 60 req/min)
...
| `dashboard_limit` | 60 req/min | Панель администратора |
...
dashboard → depends on: mongo:healthy, redis:healthy, backend:healthy
nginx    → depends on: all 4 app services (healthy)
```

> [!WARNING]
> Nginx стартует **только после** health-check всех 4 прикладных сервисов. Первый старт занимает 60–90 секунд — это нормально.
...
# ADMIN_PATH подставляется через envsubst:
location /${NGINX_ADMIN_PATH}/ {
    proxy_pass http://dashboard:8003;
}
```
