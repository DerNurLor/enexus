...
| dashboard | `http://dashboard:8003` | `https://yourdomain.com/<ADMIN_PATH>/` |
...
**Base URL:** `https://yourdomain.com/<ADMIN_PATH>`

Все эндпоинты требуют JWT с правом `admin:full` (или соответствующим правом).
...
| Параметр | Тип | Описание |
|---|---|---|
| `q` | string | Поиск по имени/username |
| `role` | string | Фильтр по роли |
| `is_blocked` | bool | Только заблокированные |
| `page` | int | Страница (default 1) |
| `per_page` | int | Записей на странице (default 50) |
  ...
  "users": [
    {
      ...
      "is_blocked": false,
      "daily_requests": null,
      "last_active": "2026-03-09T14:00:00",
...
#### `PATCH /dashboard/api/admin/users/{user_id}`
...
Список всех чатов (пользователей и групп).

#### `GET /dashboard/api/admin/chats/{chat_id}/messages`

История переписки конкретного чата.
...
| Параметр | Тип | Описание |
|---|---|---|
| `rating` | like \| dislike | Фильтр по оценке |
| `status` | pending \| rated | Фильтр по статусу |
| `search` | string | Поиск по тексту / ERR-ID |
| `page` | int | Страница |
  ...
  "items": [
    {
      ...
      "message_id": 42,
      "tg_id": 123456789,
      "user_text": "Расписание ИСС-22 на завтра",
      "bot_text": "📅 Вторник, 10 марта...",
      "rating": "like",
      "status": "rated",
...
#### `GET /dashboard/api/admin/support`

| Параметр | Тип | Описание |
|---|---|---|
| `status` | open \| answered \| closed | Фильтр по статусу |
| `category` | bug \| question \| suggestion | Фильтр по категории |
| `page` | int | |

#### `POST /dashboard/api/admin/support/{ticket_id}/reply`

```json
{"text": "Спасибо за обращение! Проблема исправлена в версии 2.0.1"}
```

#### `POST /dashboard/api/admin/support/{ticket_id}/close`

```json
{"reason": "Решено", "hidden": false}
```
...
#### `GET /dashboard/api/admin/logs/errors`

| Параметр | Тип | Описание |
|---|---|---|
| `level` | ERROR \| WARNING | Фильтр по уровню |
| `error_id` | string | Поиск по ERR-ID (напр. `ERR-ABCD12`) |
| `search` | string | Полнотекстовый поиск |
| `tg_id` | int | Ошибки конкретного пользователя |
  ...
  "items": [
    {
      ...
      "level": "ERROR",
      "error_id": "ERR-ABCD12",
      "message": "[ERR-ABCD12] KeyError: 'groupSchedule'",
      "traceback": "...",
      "tg_id": 123456789,
      "intent": "group_schedule",
      "timestamp": "2026-03-09T14:05:00"
    }
  ]
}
```
...
#### `GET /dashboard/api/admin/analytics?days=30`
...
```json
{
  ...
  "error_rate": 0.0034,
  "top_intents": [
    {"intent": "group_schedule", "count": 5200},
    {"intent": "free_rooms", "count": 2100},
    {"intent": "teacher_now", "count": 1800}
  ],
  "daily_stats": [
    {"date": "2026-03-09", "messages": 420, "users": 180, "errors": 2}
  ]
}
```
...
- [ ] Найти тикеты поддержки со статусом `open` через Dashboard API
