...
def rate_limit(max_calls: int, period: int):
    """Простой декоратор-фабрика поверх существующего rate_limit_user."""
    from functools import wraps
    from fastapi import Request
    from app.core.ratelimit import check_rate_limit, _bucket, _get_ip

    def decorator(func):
        @wraps(func)
        async def wrapper(request: Request, *args, **kwargs):
            ip = _get_ip(request)
            key = f"rl:endpoint:{func.__name__}:{ip}:{_bucket()}"
            await check_rate_limit(key, max_calls, period)
...
async def list_chats(
    ...
    pipeline = [
        {'$sort': {'timestamp': -1}},
        {'$limit': 5000},
        # Drop legacy docs with no chat_id (they'd create a null bucket)
        ...
        {'$group': {
            ...
            'msg_count':  {'$sum':   1},
            'last_text':  {'$last':  '$text'},
            # For private chats: chat_id == tg_id, used for auth_users lookup
            'tg_id':      {'$first': '$tg_id'},
        }},
        {'$sort': {'last_msg': -1}},
    ]

    # Type filter is inserted before the null-chat_id $match
    if type:
    ...
    # Bulk-load AuthUser records for all private chats so we can show real names.
    # For private chats, chat_id == tg_id — one lookup covers everyone.
    private_tg_ids = [
        c['_id'] for c in chats
        if (c.get('chat_type') or 'private') not in ('group', 'supergroup', 'channel')
    ]
    auth_users_list = await AuthUser.find(
        {'tg_id': {'$in': private_tg_ids}}
    ).to_list() if private_tg_ids else []
    auth_map = {u.tg_id: u for u in auth_users_list}

    result = []
    for c in chats:
        cid       = c['_id']
        s         = settings_map.get(cid)
        chat_type = c.get('chat_type') or 'private'

        # ── Display title ──────────────────────────────────────────────────────
        # Priority order:
        #   1. ChatSettings.title  — always set for groups/channels by the bot
        #   2. AuthUser name       — for private chats, looked up from auth_users
        #   3. @username           — from AuthUser if name fields are empty
        #   4. tg:{tg_id}          — last resort
        if s and s.title:
            display_title = s.title
        elif chat_type not in ('group', 'supergroup', 'channel'):
            # Private chat: look up in auth_users (populated on every message)
            au = auth_map.get(cid)
            if au:
                name = ' '.join(p for p in [au.first_name, au.last_name] if p).strip()
                display_title = name or (f'@{au.username}' if au.username else f'tg:{cid}')
            else:
                display_title = f'tg:{cid}'
        else:
            # Group/supergroup/channel with no ChatSettings yet
            display_title = f'chat:{cid}'

        # ── Username field ─────────────────────────────────────────────────────
        # Only show a @username when it belongs to the CHAT itself (groups/channels).
        # Never leak a user's personal @username into the group row.
        if chat_type not in ('group', 'supergroup', 'channel'):
            # Private chat: expose the user's own @username for display
            au = auth_map.get(cid)
            chat_username = au.username if au else None
        else:
            # Group/channel: only use the group's own username from ChatSettings
            chat_username = s.username if s else None
...
async def get_messages(
    ...
    # NOTE: Telegram group/supergroup chat_ids are negative integers by design.
    # Do NOT reject negative values here.
    # Only reject zero, which is never a valid Telegram chat_id.
    if chat_id == 0:
        raise HTTPException(status_code=400, detail="chat_id must not be zero")
