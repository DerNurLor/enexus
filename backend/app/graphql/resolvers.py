...
from app.search.service import normalize_query, build_mongo_search, normalize_group_name, rank_group_candidates
...
async def resolve_group_schedule(
    ...
    if gid is None and group_name:
        import re as _rg
        # 1. Exact case-insensitive match (anchored — avoids "п-ИСС-б-о-22-2" etc.)
        g = await Group.find_one({"name": {"$regex": f"^{_rg.escape(group_name)}$", "$options": "i"}})
        # 2. Anchored prefix match (group_name is already canonical "исс-б-о-22-2")
        if g is None:
            g = await Group.find_one({"name": {"$regex": f"^{_rg.escape(group_name)}", "$options": "i"}})
        # 3. Normalized separators
        if g is None:
            norm = _rg.sub(r'[\s_]+', '-', group_name.strip())
            if norm != group_name:
                g = await Group.find_one({"name": {"$regex": f"^{_rg.escape(norm)}", "$options": "i"}})
        # 4. Fuzzy: use dept prefix + year to find candidate pool, then rank with branch penalty
        if g is None:
            core = _rg.sub(r'[\s\-_]', '', group_name.lower())
            prefix_m = _rg.match(r'^([а-яёa-z]+)', group_name.lower())
            prefix_str = prefix_m.group(1)[:6] if prefix_m else core[:4]
            candidates = await Group.find(
                {"name": {"$regex": f"^{_rg.escape(prefix_str)}", "$options": "i"}}
            ).to_list(50)
            if candidates:
                try:
                    from rapidfuzz import process as fz, fuzz as fzf
                    norm_map = {_rg.sub(r'[\s\-_]', '', c.name.lower()): c for c in candidates}
...
async def resolve_teacher_schedule(
    ...
    if tid is None and teacher_name:
        ...
        if t is None:
            ...
            # e.g. "Щербины" → try "Щербин", "Щербин", etc.
            prefixes_to_try = []
        ...
        # ── Last-resort: phonetic fuzzy over broader pool ─────────────────
        # Handles typos/spelling variants that defeat prefix matching,
        # e.g. "Безпалько" (user typo) vs "Беспалько" (DB) — differ at char 3.
        if t is None and len(surname_raw) >= 4:
            # Search by first letter only to get a broad candidate pool
            first_letter = surname_raw[0].upper()
            pool = await Teacher.find(
                {"full_name": {"$regex": f"^{_r.escape(first_letter)}", "$options": "i"}}
            ).to_list(200)
            if pool:
                try:
                    from rapidfuzz import process as fz, fuzz as fzf
                    norm_q = _r.sub(r'[\s\-_]', '', surname_raw.lower())
                    # Compare query against SURNAME ONLY (first word of full_name)
                    def _surname_norm(teacher):
                        return _r.sub(r'[\s\-_]', '', teacher.full_name.split()[0].lower())
                    norm_map = {_surname_norm(c): c for c in pool}
                    best = fz.extractOne(norm_q, list(norm_map.keys()), scorer=fzf.WRatio)
                    if best and best[1] >= 70:   # higher threshold for broad pool
                        t = norm_map[best[0]]
                except ImportError:
                    pass
...
async def resolve_search(
    ...
    async def _fetch():
        ...
        groups = await Group.find(
            {"name": {"$regex": f"^{_r.escape(canonical_g)}", "$options": "i"}}
        ).limit(20).to_list()
        ...
        if not groups:
            pm = _r.match(r"^([а-яёa-z]+).*?(\d{2})", canonical_g)
            if pm:
                dept, year = _r.escape(pm.group(1)), _r.escape(pm.group(2))
                groups = await Group.find(
...
async def resolve_trigger_scrape(mode: str = "incremental") -> ScrapeResultType:
    ...
    asyncio.create_task(NCFUScraper(triggered_by="api", mode=mode).run())
    return ScrapeResultType(status="triggered", mode=mode, triggered_at=_now_iso())
